...
The manifest distinguishes `r_reference`, `native_invariant`, and `nondeterministic` correctness policies. It marks API-overhead and nondeterministic tasks as `non_comparable` for the primary aggregate while retaining them in task-level reports. Runner configurations and task specs must match both manifests before timing starts. Run `Rscript check_coverage.R` for the focused preflight; it runs the evidence, harness-trust, and source-ledger negative suites plus the R task-spec check without native builds. `run_benchmarks.R` runs this preflight before it creates measured artifacts, and `build_all.sh` repeats all three suites before native compilation.
