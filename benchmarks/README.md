...
Runner JSON files own executable symbols, not applicability. `optional_tasks` is generated in memory from non-executable evidence cells and checked against the export map. The R map is additionally split into timed pure-R and optimized rows while retaining reference-only functions for native correctness. Fixture semantics now support the nine Tier A and 47 Tier B product classifications, but every fixture row remains timing-ineligible until a source-matched rebuilt measurement. Legacy diagnostic execution can still collect timings, but those timings cannot support a product claim. Remaining work is detailed repair and classification of the historical task rows.
