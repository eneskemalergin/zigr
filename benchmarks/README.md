...
- Timing: 100 iterations via `microbenchmark` after 10 warm-up iterations
- RSS: peak from `/proc/self/status VmPeak`
- Cold start: single run before warm-up, logged separately
- Adaptive convergence: not yet implemented (fixed `times=100`)
- GC control: R's GC runs between tasks via `bench::mark` defaults. No explicit `gc(full=TRUE)` insertion
