...
Runner JSON files own executable symbols, not applicability. `optional_tasks` is generated in memory from non-executable evidence cells and checked against the export map. The R map is additionally split into timed pure-R and optimized rows while retaining reference-only functions for native correctness. The frozen classifications deliberately make no row eligible as accepted comparative timing evidence and no row Tier A. Legacy diagnostic execution can still collect timings, but those timings cannot support a product claim. Remaining work covers the exact source-to-artifact ledger, shared fixture proof, and detailed row repair.

The manifest distinguishes `r_reference`, `native_invariant`, and `nondeterministic` correctness policies. It marks API-overhead and nondeterministic tasks as `non_comparable` for the primary aggregate while retaining them in task-level reports. Runner configurations and task specs must match both manifests before timing starts. Run `Rscript check_coverage.R` for the focused preflight; it runs the evidence and harness-trust negative suites plus the R task-spec check without native builds. `run_benchmarks.R` runs this preflight before it creates measured artifacts, and `build_all.sh` repeats both negative suites before native compilation.
...
`run_benchmarks.R` creates a schema-2 run manifest and canonical input recipe artifact, captures the selected source and artifact identities, runs every selected runner in validation-only mode, and only then starts measured runner subprocesses. It validates the completed artifacts and exports comparisons from that one directory. Failed or interrupted runs remain `incomplete` and cannot be exported or promoted; a later project run marks `running` manifests older than six hours as incomplete while retaining their artifacts. Existing root-level CSVs are legacy evidence.

Each run manifest records a source-tree digest, host and CPU identity, R and Zig versions, declared target, optimization and CPU features, SEXP ABI selection, BLAS details and thread settings, locale, relevant environment variables, runner configuration, all configured shared-library fingerprints, the canonical input identity, and one normalized disposition per runner and task. Set `ZIGR_TARGET`, `ZIGR_OPTIMIZE`, `ZIGR_CPU_FEATURES`, and `ZIGR_SEXP_ABI` when the build differs from the native `ReleaseFast` defaults.
