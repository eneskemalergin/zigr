...
- **r** (R controls): 16 AST-validated pure-R rows, 56 optimized base-R or runtime rows, and 11 explicit unrepresentable dispositions backed by reference functions in `src/r/run_all.R`
...
The source gate covers all 84 runner and fixture cells. It accepts 55 generated typed product paths, zigr's P1-approved explicit F09 fixed-schema adapter, and 23 reference or control paths. Five cells remain explicit gaps: zigr's generator has no typed F08 logical-slice parameter, pure R cannot construct F10 native state, and cpp11 0.5.5 cannot express F07 complex vectors, the complete F10 generated typed-method and tag contract, or the complex-bearing F12 output set. No product gap is filled with raw FFI or zigr's untyped `R.SEXP` escape hatch.

The current fixture gate proves that each supported cell has the declared source path, retained generated artifacts where the tool emits them, package registration, load and reload behavior, and a live public-path smoke contract. It does not promote timing evidence. Exhaustive valid and invalid vectors, encoding and missing-value detail, ALTREP access instrumentation, pointer misuse, repeated recovery, and finalizer counts remain outside this gate.
...
The manifest distinguishes `r_reference`, `native_invariant`, and `nondeterministic` correctness policies. It marks API-overhead and nondeterministic tasks as `non_comparable` for the primary aggregate while retaining them in task-level reports. Runner configurations and task specs must match both manifests before timing starts. Run `Rscript check_coverage.R` for the focused preflight; it runs the evidence, harness-trust, source-ledger, and product-fixture source suites plus the R task-spec check without native builds. `run_benchmarks.R` runs this preflight before it creates measured artifacts. `build_all.sh` repeats the source gates before native compilation, installs the five product fixtures, and runs the isolated live fixture gate.
