...
Seven active runner backends. The historical report below records an earlier six-runner canonical run. Raw samples and generated reports remain ignored local evidence. Promotion validates the complete evidence set and writes only the small, trackable `results/CANONICAL_RUN.json` acceptance receipt. The current receipt is an explicitly labeled migration of the last accepted v1 promotion manifest; it preserves that manifest's schema, acceptance time, and MD5. Its 44-task comparison stays separate from the 26 boundary rows and 11 representation rows. extendr uses raw FFI wrappers for four tasks because its R-object wrapper double-panicked there.
...
The source gate covers all 84 runner and fixture cells. It accepts 46 generated typed product paths, ten generated public adapters, and 23 reference or control paths. The adapters are zigr's explicit F09 fixed-schema path and nine Rcpp entries whose annotated `Rcpp::RObject` parameters apply exact Rcpp type guards before constructing typed Rcpp views. Five cells remain explicit gaps: zigr's generator has no typed F08 logical-slice parameter, pure R cannot construct F10 native state, and cpp11 0.5.5 cannot express F07 complex vectors, the complete F10 generated typed-method and tag contract, or the complex-bearing F12 output set. No product gap is filled with raw FFI or zigr's untyped `R.SEXP` escape hatch.
...
The evidence matrix now contains nine exact Tier A product cells, 47 valid Tier B strategy cells, 23 Tier C reference or control cells, and five gaps. F01 and F12 are the only Tier A groups. F02, F05, F07, and F08 remain semantically correct but are Tier B because the cross-product group contains Rcpp's strict public adapter boundary rather than a typed generated parameter. F01 through F10 and F12 are admitted for timing after same-artifact correctness. F11 remains a correctness-only lifecycle gate, and the five gaps remain untimed.

## Task matrix

83 manifest rows: 44 core tasks (`01` through `43`, with `07a`/`07b` splitting task 07), two integration diagnostics (`48` and `49`), 26 boundary rows (`50` through `75`), and 11 representation rows (`76` through `86`).
...
Runner JSON files own executable symbols, not applicability. `optional_tasks` is generated in memory from non-executable evidence cells and checked against the export map. The R map is additionally split into timed pure-R and optimized rows while retaining reference-only functions for native correctness. Timing is enabled only for executable evidence cells and that admission is checked again immediately before each timed task or fixture. Old timings still cannot support a current product claim because their source, artifact, input, and evidence identities predate the current contracts.

The historical task matrix is now classified cell by cell: 14 Tier A product cells, nine Tier B strategy cells, 142 Tier C R or C controls, 203 Tier D diagnostics, and 213 explicit gaps. The exact Tier A groups are the matching zigr and cpp11 generated paths for tasks 50, 52, 54, 56, 58, 60, and 64. Tier B retains zigr tasks 62, 66, 68, 70, 72, and 74 plus cpp11 tasks 42, 66, and 70. cpp11 task 72 is a source-valid product path but remains diagnostic because its state contract does not match the generated-method comparison. Of the 70 registered C rows, the seven native-invariant tasks are diagnostic controls and the other 63 are kernel controls. The legacy Rcpp, extendr, and Savvy rows remain implementation diagnostics rather than product evidence. The 368 executable cells are timing-eligible after same-artifact correctness, while all 213 gaps remain ineligible and visible.
...
Each runner record keeps both the authored recipe and the resolved top-level build invocation, including its executable, arguments, working directory, relevant environment overrides, and whether that invocation ran in the current run. Normalized fixtures record their own resolved build invocation or invocation sequence rather than borrowing the historical runner command. A prebuilt run records `executed_in_run=false`; it does not claim that the artifact was rebuilt from the recorded source.
...
The manifest distinguishes `r_reference` and `native_invariant` correctness policies and retains the frozen `nondeterministic` vocabulary for compatibility. API-overhead tasks remain `non_comparable` for the primary aggregate while staying visible in task-level reports. Task 43 now uses a deterministic seed and compares both returned values and post-call R RNG state; it is no longer validated as an unconstrained nondeterministic result. Runner configurations and task specs must match both manifests before timing starts. Run `Rscript check_coverage.R` for the full no-native trust suite. Filtered benchmark commands use `Rscript check_coverage.R --quick` internally to validate manifests, runner configuration, and selected task specs without repeating every negative suite. Unfiltered runs retain the full gate. `build_all.sh` only builds runners and installs fixtures; correctness, registration, lifecycle, and live fixture validation remain owned by the benchmark parent before timing. Rust runners and fixtures share the ignored `tmp/cargo-target` cache by default. Set `CARGO_TARGET_DIR` to use another cache location.

Input generators remain in one place in `runner_subprocess.R`. Before runner execution, that source derives one seed per selected task from the run seed, task ID, and fixture version, then writes a canonical recipe manifest. Its structural fingerprint covers values, types, attributes, encodings, classes, dimensions, ALTREP intent, and RNG state; the recipe also records its mutation policy. Every runner receives the same manifest path and digest. Each executable task verifies its recipe on the first real phase materialization instead of performing a separate full-task regeneration pass.

## Correctness validation

Before any runner records timing, a validation-only pass checks all selected rows. Every native task is validated against the declared R reference or its manifest result contract, and every executable R row validates its own result contract. The native path and reference receive separate deterministic argument instances. The return value comparison checks type, attributes, recursive list structure, numeric tolerance, exact missing-value kind (NA versus NaN), and character encoding. Reference errors, native errors, missing references, contract mismatches, input mutation, registration errors, artifact drift, and value mismatches stop the run before timing. Timing subprocesses consume those retained correctness artifacts rather than repeating the calls. Normalized fixtures reuse immutable phase inputs; stateful receivers and ALTREP inputs remain fresh for every sample.

Correctness, reference, cold, warmup, and timed phases use separate instances. Immutable inputs may be reused within warmup or timed sampling only when their fingerprints match before and after sampling. Sort and attribute mutation receive fresh arguments outside every timed sample, RNG work resets the declared stream outside every sample, and external-state methods construct a fresh registered receiver outside every sample. Fixture construction, duplication, and reset work are not timed.
...
# Retain the complete correctness stage without timing files
...
# Keep the ten newest inactive local runs, plus the canonical and running runs
Rscript run_benchmarks.R --prune-runs=10
...
# Exercise the complete promotion gate without changing the acceptance receipt
...
`run_benchmarks.R` creates a schema-2 run manifest with an environment-schema-3 tool and source ledger plus the canonical input recipe artifact. It runs every selected fixture and task runner in validation-only mode and retains structured correctness rows before starting measured subprocesses. Pass `--correctness-only` to stop after that complete source-matched validation stage without creating timing artifacts. A timed run measures normalized fixtures before the historical task matrix, validates both artifact sets, and rechecks the source tree immediately before marking the run complete. Report generation is explicit and separate from measurement completion. Failed or interrupted runs remain `incomplete` and cannot be exported or promoted; a later project run marks `running` manifests older than six hours as incomplete while retaining their artifacts. Existing root-level CSVs are legacy evidence.
...
The pipeline writes structured task and fixture correctness artifacts under `correctness/`. Timed runs also write per-task timing CSVs, normalized fixture timing CSVs, per-runner summaries, and fixture summaries under the same run directory. Completion seals the exact raw, summary, input, correctness, source, tool, generated-glue, dependency, artifact, policy, and disposition identities in the run manifest. For a classified seven-runner run, explicit comparative export writes `product_metrics.csv`, `strategy_metrics.csv`, `r_baseline_metrics.csv`, `control_metrics.csv`, `diagnostic_metrics.csv`, `capability_matrix.csv`, `safety_results.csv`, and their `report_manifest.json`. Each role remains separate, and product groups retain linked R and registered-C evidence. Older compatible runs retain the legacy shared-intersection `task_comparisons.csv` and `category_metrics.csv` outputs. Boundary export writes `boundary_metrics.csv`, `boundary_budgets.csv`, and `representation_budgets.csv`. `analysis/summarize.R` writes `analysis_summary.csv` beside the selected run. Promotion validates the completed run against the current tree, regenerates every report in an isolated temporary directory, reruns lifecycle proof, and records report digests, row coverage, correctness identities, budget status, and completion seals in `results/CANONICAL_RUN.json`. Raw samples and generated reports stay local and ignored rather than accumulating in repository history.

## Generated boundary budgets

The original boundary budget baseline is complete focused run `20260711T232455Z-pid2`: rebuilt ReleaseFast libraries, native target, Zig 0.16.0, R 4.6.1, and budget policy `2026-07-11-1`. C passed all 26 boundary rows and reported the 11 zigr-only representation rows as `N/A`; R and zigr passed all 37 selected rows. Policy `2026-07-15-1` retains every class limit and every other representation limit, but changes the borrowed raw-view ceiling from `0.01 ms` to `0.02 ms`. The phase-isolated harness measures explicit prepared-call evaluation and error capture instead of the earlier `microbenchmark(eval(expr))` path; repeated source-matched full runs place the unchanged 256 KiB raw-view workload between `0.0140 ms` and `0.0148 ms`, while its explicit-copy companion remains near `0.16 ms`. The new value is a regression ceiling for the current measurement protocol, not a speed claim. Runs recorded under the old policy cannot be promoted under the new policy.
...
- Legacy category reports: `category_metrics.csv` groups shared-intersection results into kernels, boundary, ALTREP, R runtime, and synthetic API categories.
